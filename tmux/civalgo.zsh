...
if tmux has-session -t civalgo 2>/dev/null; then
    ...
    tmux send-keys -t civalgo:dev-docker.0 'cd /Users/markjarjour/Documents/gitter/borpa-business/civalgo/portal' Enter
    tmux send-keys -t civalgo:dev-docker.2 'cd /Users/markjarjour/Documents/gitter/borpa-business/civalgo/sombra' Enter
    tmux send-keys -t civalgo:dev-docker.3 'cd /Users/markjarjour/Documents/gitter/borpa-business/civalgo/sombra' Enter
    ...
    tmux send-keys -t civalgo:dev-docker.2 '
    check_docker() {
    ...
    tmux send-keys -t civalgo:portal 'cd /Users/markjarjour/Documents/gitter/borpa-business/civalgo/portal; tmux wait-for -S portal-cd-complete' Enter
    tmux wait-for portal-cd-complete

    print_success "Portal windows started..."

    tmux new-window -t civalgo -n 'sombra'
    tmux send-keys -t civalgo:sombra 'cd /Users/markjarjour/Documents/gitter/borpa-business/civalgo/sombra; tmux wait-for -S sombra-cd-complete' Enter
    ...
    NO_MIGRATION_MESSAGE="Already up to date"

    # Run git pull, npm install, and database migration
    echo -e "${SOMBRA_COLOR}Running git pull, npm install, and database migration...${NC}"
    tmux send-keys -t civalgo:sombra 'git pull && npm i && npm run db:migrate; tmux wait-for -S sombra-db-complete' Enter
    tmux wait-for sombra-db-complete

    echo -e "${SOMBRA_COLOR}Git pull, npm install, and database migration complete...${NC}"

    # Check if the migration output mentions Knex
    if tmux capture-pane -p -t civalgo:sombra | grep -q "$NO_MIGRATION_MESSAGE"; then
        echo -e "${SOMBRA_COLOR}No Knex migration detected. Continuing...${NC}"
    else
        echo -e "${SOMBRA_COLOR}Knex migration detected. Killing process in pane 0.2...${NC}"

        # Kill the process running in pane 0.2 and wait for it to exit, then start it again
        tmux send-keys -t civalgo:0.2 'pkill -f "npm run dev"; while kill -0 $PID 2>/dev/null; do sleep 1; done; npm run docker-start' Enter
    fi
